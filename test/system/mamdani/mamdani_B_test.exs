defmodule MandaniBTest do
  ...
  test "4 controllers test" do
    ...
    r1 = fn [at1, con] -> (at1 ~> "mb" &&& at1 ~> "mb") >>> con ~> "mb"
    end
    r2 = fn [at1, con] -> (at1 ~> "ms" &&& at1 ~> "ms") >>> con ~> "mb"
    end
    r3 = fn [at1, con] -> (at1 ~> "ze" &&& at1 ~> "ze") >>> con ~> "mb"
    end
    r4 = fn [at1, con] -> (at1 ~> "ps" &&& at1 ~> "ps") >>> con ~> "ms"
    end
    r5 = fn [at1, con] -> (at1 ~> "pb" &&& at1 ~> "pb") >>> con ~> "ms"
    end
    ...
    r1 = fn [at1, con] -> (at1 ~> "mb" &&& at1 ~> "mb") >>> con ~> "pb"
    end
    r2 = fn [at1, con] -> (at1 ~> "ms" &&& at1 ~> "ms") >>> con ~> "ps"
    end
    r3 = fn [at1, con] -> (at1 ~> "ze" &&& at1 ~> "ze") >>> con ~> "ze"
    end
    r4 = fn [at1, con] -> (at1 ~> "ps" &&& at1 ~> "ps") >>> con ~> "ms"
    end
    r5 = fn [at1, con] -> (at1 ~> "pb" &&& at1 ~> "pb") >>> con ~> "mb"
    end
...
#     compute({srm_res_pid, srm_vis_pid, sw_dens_pid, sw_vis_pid}, [-2.53, -0.04, -18.9])
  ...
  defp compute({srm_res_pid, srm_vis_pid, sw_dens_pid, sw_vis_pid},[residual_error, density_error, viscosity_error]) do
    System.compute(srm_res_pid, [4.1])
